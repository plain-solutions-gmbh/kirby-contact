...
class Autoloader
{
    ...
    const ALLOWED_IDS = ['api', 'areas', 'assetMethods', 'authChallenges', 'blockMethods', 'blockModels', 'blocksMethods', 'blueprints', 'cacheTypes', 'collections', 'commands', 'components', 'controllers', 'collectionFilters', 'collectionMethods', 'fieldMethods', 'fileMethods', 'filePreviews', 'fileTypes', 'filesMethods', 'fields', 'hooks', 'layoutMethods', 'layoutColumnMethods', 'layoutsMethods', 'pages', 'options', 'pageMethods', 'pagesMethods', 'pageModels', 'permissions', 'routes', 'sections', 'siteMethods', 'snippets', 'structureMethods', 'structureObjectMethods', 'tags', 'templates', 'thirdParty', 'translations', 'userMethods', 'userModels', 'usersMethods', 'validators'];
    ...
    public array $tasks = [
        ...
        /**
         * Inject translations right at the begining.
         * So it could be used already in classes
         */
        'translations' => [
    ...
    ): void
    {
        ...
        //Load classes
        $this->registerTranslations($cache['translations']);
    ...
    private function setValue(string|array $key_chain, string $file, bool $read = true): void
    {
        ...
        //Skip unwanted keys
        if (in_array($key_chain[0] ?? null, static::ALLOWED_IDS) === false) {
    ...
    /**
     * Load translations from folder I18n
     * 
     * @param string $path 
     * @return void 
     */
    public function loadTranslations(
        ...
        $this->filesWalker($path, function ($file) use ($path) {
            $key = $this->keyFromPath($path, $file, '/');
            $this->translations[$key] = Data::read($file);
        });
     ...
     * @return void 
     */
    private function registerTranslations(?array $translations = null): void
    {
        $translations ??= $this->translations;
        App::instance()->extend(compact('translations'));
    }
